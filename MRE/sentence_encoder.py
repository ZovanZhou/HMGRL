...
class BERTSentenceEncoder(tf.keras.models.Model):
    ...
    @tf.function
    def call(self, s_ind, s_seg, s_l_ind, s_l_seg, u_l_ind, u_l_seg, head_idx, tail_idx):
        h_sentence = self.bert_embedding(s_ind, s_seg)
        h_seen_label = tf.reduce_mean(self.bert_embedding(s_l_ind, s_l_seg), axis=1)
        h_unseen_label = tf.reduce_mean(self.bert_embedding(u_l_ind, u_l_seg), axis=1)
        ...
        h_relation = (h_head + h_tail) / 2.0
        return h_sentence, h_entity, h_relation, h_seen_label, h_unseen_label


class ZSBERTSentenceEncoder(tf.keras.models.Model):
    def __init__(self, bert_path: str, fine_tune: bool = True, hidden_size: int = 768):
        super(ZSBERTSentenceEncoder, self).__init__()
        self.bert_embedding = BERTEmbedding(bert_path, fine_tune)
        self.ln1 = Dense(hidden_size, activation="tanh")
    ...
    @tf.function
    def call(self, s_ind, s_seg, l_ind, l_seg, head_idx, tail_idx):
        h_sentence = self.bert_embedding(s_ind, s_seg)
        h_label = tf.reduce_mean(self.bert_embedding(l_ind, l_seg), axis=1)
        batch_size = tf.shape(h_sentence)[0]
        h_head = self.get_tensor_by_index(h_sentence, batch_size, head_idx)
        h_tail = self.get_tensor_by_index(h_sentence, batch_size, tail_idx)
        h_entity = self.ln1(tf.concat([h_head, h_tail], axis=-1))
